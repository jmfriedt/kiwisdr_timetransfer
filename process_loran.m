...
for dirnum=1:length(dirname)
  dirname(dirnum).name
  dlist=dir([dirname(dirnum).name,'/*100000*QTR*wav']);
  for l=1:length(dlist)
    ...
    if (isinf(fs)==0)
      ...
      if (dirnum>1)
        if dt(dirnum)+weeks*86400*7<dt(dirnum-1)
           weeks=weeks+1;
        end
      end
      dt(dirnum)=dt(dirnum)+weeks*86400*7;
      loran_mag=abs(z);
      loran_time=t-floor(t(1))+mod(((dt(dirnum)-dinit)/GRI/2),1)*GRI*2;
      kinit=find(loran_time>1.16);kinit=kinit(1);
      kstop=find(loran_time>1.20);kstop=kstop(1);
      k=find(loran_mag(kinit:kstop)>max(loran_mag(kinit:kstop))/2);k=k(1)+kinit-1;
      if (isempty(k)==0)
        for griindex=0:40
