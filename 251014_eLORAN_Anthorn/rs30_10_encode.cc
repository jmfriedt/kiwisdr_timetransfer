...
DEFUN_DLD(rs30_10_encode, args, nargout, "in=[0x79,0x08,0x01,0x58,0x00,0x1C,0x73,0x2F,0x44,0x26];out=rs30_10_encode(in)")
{int nargin=args.length();
 ...
 if (nargin != 1) {print_usage();nargout=0;}
       ...
       din=(unsigned char*)malloc(dlen);
       dout=(unsigned char*)malloc(127);
       for (int k=0;k<in.numel();k++) din[k]=(int)inv[k];
       encode_rs30_10(din, dlen, dout, 127);
       for (int k=0;k<N;k++) matout(0,k)=(double)(dout[127-N+k]);
       free(din);free(dout);
      }
 retval(0)=matout;
 return retval;
}
#endif        // Octave library
...
i=codeword_size-N+5;
printf("codeword[%d] was %02hhx -> 42: ",i,r->index_of[codeword[i]]);
codeword[i]=0x42;
